...
export class DashboardService {
  ...
  async getOverview() {
    ...
    const totalAssets = await this.prisma.asset.count({ where: { is_active: true } });
    ...
    const formattedOccupancy = locationOccupancy.map(loc => ({
      ...
      is_overloaded: (loc._count.assets_current + loc._count.assets_allocated) >= loc.max_capacity
    }));
